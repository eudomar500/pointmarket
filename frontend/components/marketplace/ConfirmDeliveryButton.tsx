...
import { useConfirmDelivery } from "@/lib/hooks/useConfirmDelivery";
...
export default function ConfirmDeliveryButton({
  ...
  const { confirmDelivery, pending } = useConfirmDelivery();
  ...
  const handleConfirm = async () => {
    setErrorMsg(null);
    try {
      const hash = await confirmDelivery(tradeId);
        ...
        {submittedHash ? (
          <div>
            <p className="text-sm text-[var(--text-secondary)] mb-4">
              Delivery confirmation submitted. Track its progress in the drawer
              at the bottom right, or open it on the explorer. The seller
              receives funds once the transaction finalizes.
            </p>
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-6">
              Confirming delivery closes the trade and releases the escrow to
              the seller, minus the protocol fee. This action is final. If you
              did not actually receive the item, open a dispute instead.
            </p>
